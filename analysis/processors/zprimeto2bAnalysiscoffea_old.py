...
## Below is OLD code
        ...
        if not isData:
            ...
            trig = {
                'cr': get_ele_trig_weight(ak.sum(leading_e.eta+leading_e.deltaEtaSC, axis=-1), ak.sum(leading_e.pt,axis=-1)),
                'sr': get_ele_trig_weight(ak.sum(leading_e.eta+leading_e.deltaEtaSC, axis=-1), ak.sum(leading_e.pt,axis=-1)),
            }
            trig_err = {
                'cr': get_ele_trig_err(ak.sum(leading_e.eta+leading_e.deltaEtaSC, axis=-1), ak.sum(leading_e.pt,axis=-1)),
            ...
            ###
            ids = {
                'cr': get_ele_tight_id_sf(ak.sum(leading_e.eta, axis=-1), ak.sum(leading_e.pt, axis=-1)),
                'sr': get_ele_tight_id_sf(ak.sum(leading_e.eta, axis=-1), ak.sum(leading_e.pt, axis=-1)),
            }
            ids_err = {
                'cr': get_ele_tight_id_err(ak.sum(leading_e.eta, axis=-1), ak.sum(leading_e.pt, axis=-1)),
            ...
            btagUp = {}
            btagDown = {}
            btag['cr'],   btagUp['cr'],   btagDown['cr'] = get_deepflav_weight['medium'](j_good_clean,j_good_clean.pt, j_good_clean.eta, j_good_clean.hadronFlavour, '+1', )
        ...
        jet_trig_sr = {
            '2016':{
                'DoubleJet90_Double30_TripleBTagCSV_p087': 0.10,
                'HLT_QuadJet45_TripleBTagCSV_p087': 0.12
            },
            '2017':{
                'PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0': 0.10,
            },
            '2018':{
                'DoublePFJets40_CaloBTagCSV': 0.10,
                'PFHT180': 0.12
            }
        }
        jet_trig_cr = {
...
#                continue
#            prescale_trig = events.HLT[path] * jet_trig_sr[self._year][path]
#            triggers = triggers | (~events.HLT[path] * jet_trig_sr[self._year][path])
...
#            if path not in events.HLT.fields:
#                continue
#            prescale_trig = events.HLT[path] * jet_trig_cr[self._year][path]
#            triggers = triggers | (~events.HLT[path] * jet_trig_cr[self._year][path])
...
#                continue
#            prescale_trig = events.HLT[path] * jet_trig_sr[self._year][path]
#            triggers = triggers | prescale_trig
                ...
                if region == 'sr':
                    for path in self._jet_triggers_sr[self._year]:
                        if path in events.HLT.fields:
                            weights.add(str(path)+'_weight', events.HLT[path] * 0.10)
                            print(events.HLT[path] * 0.10)

                if region == 'cr':
                    for path1 in self._jet_triggers_cr[self._year]:
                        if path1 in events.HLT.fields:
                            weights.add(str(path)+'_weight', events.HLT[path1] * 0.10)
